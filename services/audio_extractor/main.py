...
def run_ffmpeg(input_path: str, output_path: str) -> None:
    ...
    try:
        (
            ...
            .output(
                output_path, acodec="libmp3lame", audio_bitrate="128k", loglevel="error"
            )
            .overwrite_output()
...
async def handle_video_uploaded(event: VideoUploaded, msg: RabbitMessage) -> None:
    ...
    try:
        ...
        await asyncio.to_thread(
            minio_client.fget_object, "videos", event.minio_path, temp_video_path
        )
        ...
        await asyncio.to_thread(
            minio_client.fput_object, "audio", audio_filename, temp_audio_path
        )

        # Publish Event
        output_event = AudioExtracted(
            video_id=event.video_id, audio_path=audio_filename
        )
        ...
        logger.error(
            "audio_extraction_failed", error=str(e), video_id=str(event.video_id)
        )
