...
class AudioExtracted(BaseModel):
    ...
    audio_path: str = Field(
        ..., description="Path to the extracted audio file in MinIO"
    )
    duration_seconds: Optional[float] = Field(
        None, description="Duration of the extracted audio"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Event timestamp",
...
class TranscriptReady(BaseModel):
    ...
    transcript_text: str = Field(
        ..., description="The full raw text of the transcription"
    )
    transcript_json: Dict[str, Any] = Field(
        ..., description="Full transcript from AssemblyAI including speaker labels"
    )
    speaker_segments: Optional[Dict[str, Any]] = Field(
        None, description="Parsed speaker segments with timestamps"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Event timestamp",
